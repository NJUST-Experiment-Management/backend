...
public class CourseController extends BaseController {
    ...
    public Result<?> addCourse(@RequestBody MultipartFile file, @RequestBody Course course) {
    ...
    public Result<?> getStudentList(@RequestParam Course course) {
        if (getUser().getUserType().equals("STUDENT"))
            return Result.error("-1", "无权限");
    ...
    public Result<?> addCourseArrangement(@RequestBody Course course, @RequestBody List<Date> dates, @RequestBody List<Integer> times,
                                          @RequestBody List<Room> rooms, @RequestParam Boolean isShareable) {
        if (getUser().getUserType().equals("STUDENT"))
            return Result.error("-1", "无权限");
        if (dates.size() != times.size())
            return Result.error("-1", "传递列表大小不一致");
        for (int i = 0; i < dates.size(); i++)
            arrangeService.addArrangement(course, dates.get(i), times.get(i), rooms, isShareable);
        return Result.success();
    }

    @ApiOperation("选择开放性实验")
    @PostMapping("arrangement/chooseOpenCourse")
    public Result<?> chooseOpenCourseArrangement(@RequestBody Course course, @RequestBody Date date, @RequestBody Integer time,
